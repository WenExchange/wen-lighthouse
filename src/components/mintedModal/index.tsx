    ...
    <C.Modal>
      ...
      <C.Dialog>
        ...
        <C.DialogBody>
          {props.mints.length === 1 && (
            <C.NftSingle>
              <C.Nft>
                <C.NftImage
                  src={
                    typeof props.mints[0].data === "undefined"
                      ? `${props.tokenUri}/${props.mints[0]}`
                      : props.mints[0].data.image
                  }
                ></C.NftImage>
                <C.NftTitle>
                  {config.nft_name_type === "default" && (
                    <>
                      {typeof props.mints[0].data === "undefined"
                        ? props.name + " #" + props.mints[0]
                        : props.mints[0].data.name}
                    </>
                  )}
          ...
          <C.Bottom>
            <C.Title>Mint Successful!</C.Title>

            <C.Button onClick={props.close}>OK</C.Button>
          </C.Bottom>
        </C.DialogBody>
      </C.Dialog>
    </C.Modal>
